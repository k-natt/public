...
void bring_to_front(enum rank *cards, enum rank rank)
{
	int x = 0;
	int c = 0;
	enum rank non[HAND_SIZE];
	for(int i = 0; i < HAND_SIZE; i++) {
		if(cards[i] == rank) {
			c++;
		} else {
	...
	for(int i = 0; i < c; i++) cards[i] = rank;
	while(x --> 0) cards[c+x] = non[x];
}
...
enum hand check_multis(enum rank *highs)
{
	...
	if(cc[4]) {
		int i;
		for(i = 0; counts[i] < 4; i++){}
		bring_to_front(highs, i);
		return FOUR_OF_A_KIND;
	}
	if(cc[3] && cc[2]){
		int i;
		for(i = 0; counts[i] != 2; i++){}
		bring_to_front(highs, i);
		for(i++; counts[i] < 3; i++){}
		bring_to_front(highs, i);
		return FULL_HOUSE;
